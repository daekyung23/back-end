...
function getAllDeviceQuery() {
    return `
        ...
        LEFT JOIN \`Option\` Fax ON Fax_Option.Option_ID = Fax.Option_ID AND Fax.OptionType = 'Fax'
        LEFT JOIN Option_in_device Desk_Option ON d.Device_ID = Desk_Option.Device_ID
        LEFT JOIN \`Option\` Desk ON Desk_Option.Option_ID = Desk.Option_ID AND Desk.OptionType = '데스크'
        LEFT JOIN Option_in_device Shelf_Option ON d.Device_ID = Shelf_Option.Device_ID
        LEFT JOIN \`Option\` Shelf ON Shelf_Option.Option_ID = Shelf.Option_ID AND Shelf.OptionType = '3/4단'
    `;
}
